        ...
        <>
            <nav className="relative">
                <div className={`${searchOpen || navOpen ? 'hidden lg:block' : ''}`}>
                    <MaxWidthWrapper className="py-1.5 text-center flex justify-center items-center font-regular text-xs text-custom-dark-blue md:text-sm lg:text-base overflow-x-hidden whitespace-nowrap">
                                ...
                                <div className="flex items-center gap-4 md:gap-6">
                                    ...
                                    <div className={`relative hidden lg:flex bg-custom-dark-blue bg-transparent ${searchOpen ? 'py-3 h-16' : 'delay-1000 py-0 h-0'}`}>
                                        ...
                                        <button
                                            ...
                                            className={`absolute h-8 w-8 z-20 top-1/2 transform -translate-y-1/2 transition-opacity duration-500 delay-300 ${searchOpen ? 'opacity-100' : 'delay-0 opacity-0'}`}>
                ...
                <div className="bg-custom-deep-blue">
                    <MaxWidthWrapper>
                        <div className={`fixed w-full z-50 lg:flex bg-custom-deep-blue lg:static lg:justify-around lg:items-center lg:flex-row ${navOpen || searchOpen ? 'block' : 'hidden'}`}>
                            ...
                            <ul className={`realative px-6 py-5 w-full flex flex-col gap-5 lg:flex-row lg:justify-around lg:items-center font-light text-2xl text-white capitalize lg:text-xl ${navOpen ? 'block' : 'hidden lg:flex'}`}>
                                {navLinks.map((navLink) => (
                                    <li key={navLink.title}>
                                        <Link href={navLink.href} className="relative flex transition-all hover:text-red-500">
                                            {navLink.title}
